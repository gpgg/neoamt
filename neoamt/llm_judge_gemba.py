...
GEMBA_DA_REF = 'Score the following translation from {source_lang} to {target_lang} with respect to human reference on a continuous scale 0 to 100 where score of zero means "no meaning preserved" and score of one hundred means "perfect meaning and grammar".\n\n{source_lang} source: "{source_seg}"\n{target_lang} human reference: {reference_seg}\n{target_lang} machine translation: "{target_seg}"\nScore: '
...
def main():
    ...
    if args.method == "gemba":
        ...
        for i, item in enumerate(tqdm(data)):
            ...
            prompt = GEMBA_DA_REF.format(
                source_lang=src_lang,
                target_lang=tgt_lang,
                source_seg=source_text,
                reference_seg=reference_translation,
                target_seg=candidate_translation,
            )
            ...
            for _ in range(max_tries):
                ...
                try:
                    score_str = validate_number(response)
        ...
        if args.eval_format == "amt":
            ...
            from datetime import datetime
            now = datetime.now().strftime("%Y-%m-%d-%H-%M")
            output_path = args.input_path.replace(".json", f"_llm_judge_{args.method}_{model_name}_{str(now)}.jsonl")
