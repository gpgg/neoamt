...
class BingSearchTool(BaseTool):
    ...
    def execute(self, query: str, lang=None, timeout: int = 60) -> str:
        """
    ...
    def _extract_and_format_results(self, data: Dict) -> str:
        ...
        for result in data['organic']:
            ...
            if len(snippet) > 0 and snippet not in seen_snippets:
                chunk_content_list.append({"link": link, "title": title, "snippet": snippet})
    ...
    def _format_results(self, results: Dict) -> str:
        ...
        formatted = []
        for idx, content in enumerate(results["chunk_content"][:self._max_results], 1):
            title = content["title"]
            snippet = content["snippet"]
            snippet = snippet[:self._result_length]
            formatted.append(f"Doc {idx}(Title: {title}) {snippet}\n")
...
class SearXNGSearchTool(BaseTool):
    """
    SearXNG search tool that provides web search capability with caching.
    
    This tool interfaces with the SearXNG API to perform searches.
    It includes robust caching to minimize redundant API calls and supports
    both synchronous and asynchronous cache writing modes.
    ...
    def _setup_cache_paths(self, cache_file: Optional[str]) -> None:
        ...
        if cache_file is None:
            cache_dir = pathlib.Path.home() / ".verl_cache"
            cache_dir.mkdir(exist_ok=True)
            self._cache_file = cache_dir / "searxng_search_cache.json"
        else:
            self._cache_file = pathlib.Path(cache_file)
            self._cache_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _make_request(self, query: str, timeout: int) -> requests.Response:
        ...
        r = requests.get('http://localhost:8888/', params=payload, timeout=timeout)
    ...
    def _format_results(self, results: Dict) -> str:
        ...
        """
        formatted = []
        for idx, r in enumerate(results[:self._max_results], 1):
            url = r.get("url", "").strip()
            title = r.get("title", "").strip()
            content = r.get("content", "").strip()[:self._result_length]
            
            formatted.append(f"Doc {idx}(Title: {title}) {content}\n")
...
class DenseRetrieverTool(BaseTool):
    ...
    def _setup_cache_paths(self, cache_file: Optional[str]) -> None:
        ...
        if cache_file is None:
            cache_dir = pathlib.Path.home() / ".verl_cache"
            cache_dir.mkdir(exist_ok=True)
            self._cache_file = cache_dir / "dense_retrieval_cache.json"
        else:
            self._cache_file = pathlib.Path(cache_file)
            self._cache_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    def batch_search(self, queries: List[str] = None, lang=None) -> str:
        """
        ...
        """
        results = self._batch_search(queries, lang=lang)['result']
        
        return [self._passages2string(result) for result in results]

    def _batch_search(self, queries: List[str], lang=None):
        payload = {
            "queries": queries,
            "topk": self._max_results,
            "return_scores": True,
            "lang": lang,
        }
        # return requests.post("http://fcdgx00033.cm.cluster:8001/retrieve", json=payload).json()
        return requests.post(f"{self.url}", json=payload).json()
    ...
    def execute(self, query: str, lang=None, timeout: int = 60) -> str:
        ...
        try:
            # Make API request
            # response = self._make_request(query, timeout)
            result = self.batch_search([query], lang=lang)[0]
            result = result[:self._result_length]
            ...
            # Extract search results
            # result = self._extract_and_format_results(result)
            
            # Disable cache for local dense retriever
            # Update cache
            with self._cache_lock:
                self._cache[query] = result
            ...
            error_msg = f"Dense retrieval search failed: {str(e)}"
            print(error_msg)
            return ""
    ...
    def _extract_and_format_results(self, data: Dict) -> str:
        ...
        for result in data['organic']:
            ...
            title = result.get('title', '').strip()
            if len(snippet) > 0 and snippet not in seen_snippets:
                chunk_content_list.append({"title": title, "snippet": snippet})
                seen_snippets.add(snippet)
...
class LocalBingSearchTool(BaseTool):
    def __init__(
        self,
        api_key: str,
        max_results: int = 10,
        result_length: int = 1000,
        url: str = "http://localhost:8002/bing_search",
        zone: str = "serp_api1",
        location: str = "cn",
        cache_file: Optional[str] = None,
        async_cache_write: bool = True,
    ...
    def _setup_cache_paths(self, cache_file: Optional[str]) -> None:
        ...
        if cache_file is None:
            cache_dir = pathlib.Path.home() / ".verl_cache"
            cache_dir.mkdir(exist_ok=True)
            self._cache_file = cache_dir / "local_bing_search_cache.json"
        else:
            self._cache_file = pathlib.Path(cache_file)
            self._cache_file.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _init_async_writer(self) -> None:
        ...
        self._writer_thread = threading.Thread(
            target=self._cache_writer_thread, 
            daemon=True,
            name="LocalBingSearchCacheWriter"
        )
        self._writer_thread.start()
        atexit.register(self._cleanup)
    ...
    def _make_request(self, query: str, timeout: int) -> requests.Response:
        ...
        res = requests.post(self._local_url, json=payload,)
        return res
...
class BM25RetrieverTool(DenseRetrieverTool):
    def __init__(
        self,
        url: str,
        # topk: int = 3,
        ...
        **kwargs,
        ...
        super().__init__(url, max_results, result_length, cache_file, async_cache_write, cache_refresh_interval, **kwargs)
    ...
    def execute(self, query: str, lang=None, timeout: int = 60) -> str:
        ...
        try:
            ...
            error_msg = f"bm 25 search failed: {str(e)}"
            print(error_msg)
            return ""
...
if __name__ == "__main__":
    ...
    # cache_file = ".local_bing_search_cache"
    # local_url = "http://localhost:8002/bing_search"
    # api_key = ""
    # zone = "serp_api1"
    # max_results = 5
    # location = "jp"
    # search_tool = LocalBingSearchTool(
    #     api_key=api_key,
    #     url=url,
    #     zone=zone,
    ...
    # search_tool = DenseRetrieverTool(
    #     api_key="",
    #     url="http://localhost:8003/retrieve",
    #     topk=10,
    #     cache_file=".debug_dense_retriever_cache"
    # )
    
    search_tool = BM25RetrieverTool(
        api_key="",
        url="http://localhost:8003/retrieve",
        max_results=5,
        cache_file=".debug_bm25_cache"
    )
    
    result = search_tool.execute("刀", timeout=120, lang="all")
    print("\nSearch results:\n")
    print(result)
    print(type(result))
