...
class DataProto:
    ...
    def dynamic_repeat(self, n: np.array, interleave: bool = True):
        num = len(n)
        total_n = sum([int(j) for j in n])
        if self.batch is not None:
            assert self.batch.shape[0] == num, (
                f"the length of n {num} must be equal to the batch size {self.batch.shape[0]}")
            if interleave:
                # interleave the data
                ...
                for key, tensor in self.batch.items():
                    ...
                    for i in range(tensor.shape[0]):
                        repeated_tensors[key].append(tensor[i].unsqueeze(dim=0).repeat_interleave(int(n[cur]), dim=0))
                        cur += 1
        ...
        for key, val in self.non_tensor_batch.items():
            ...
            if interleave:
                ...
                for i in range(val.shape[0]):
                    # if val[i] is not array, convert it to array
                    if not isinstance(val[i], np.ndarray):
                        for j in range(int(n[cur])):
                            v_list.append(val[i])
                    else:
                        repeated_non_tensor_batch[key].append(np.repeat(np.expand_dims(val[i], axis=0), int(n[cur]), axis=0))
                    cur += 1
                if len(v_list) > 0:
                    repeated_non_tensor_batch[key]= np.array(v_list, dtype=object)
        # r_non_tensor_batch = {key: val for key, val in repeated_non_tensor_batch.items()}
        return type(self)(
